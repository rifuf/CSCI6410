...
# Tokenize the transcription into bigrams
tokenized.data <- filtered.data %>%
  unnest_tokens(ngram, transcription, token = "ngrams", n = 2, to_lower = TRUE)

# Remove bigrams containing stop words or numbers
stop_words <- tidytext::stop_words$word
tokenized.data_filtered <- tokenized.data %>%
  filter(!(str_detect(ngram, paste(stop_words, collapse = "|")) | str_detect(ngram, "[0-9]")))

# Count unique bigrams per specialty
unique_bigrams_per_specialty <- tokenized.data_filtered %>%
  group_by(medical_specialty) %>%
  summarise(num_unique_bigrams = n_distinct(ngram))

# Print the result
unique_bigrams_per_specialty
```
...
Lemmatization is the process of reducing words to their base or dictionary form. While general-purpose lemmatizers work well for common language text, I believe they may have limitations when applied to medical data. 

This is because medical data often contain specialized terminology, acronyms, drug names, and disease-specific terms that may not be available in a general-purpose dictionary. Due to rare or infrequent words being present, general-purpose lemmatizers might not recognize these specialized terms, resulting in inaccurate lemmatization. Moreover, there may be cruicial domain-specific rules and contextual ambiguity which may get lost during lemmatization. Hence, I believe general-purpose lemmatizers might not work well for medical data. 
...
The Surgery vs. Orthopedic scatter plot indicates that many lemmas have similar frequencies in both “Surgery” and “Orthopedic.” However, there are deviations from this line, suggesting that some lemmas are more frequent in one specialty compared to the other.

The points in the Radiology vs. Orthopedic scatter plot are more spread away from the line compared to the Surgery vs. Orthopedic scatter plot. This greater spread indicates more dissimilarity in lemma frequencies between “Radiology” and “Orthopedic.”

The observed patterns align with our expectations since Surgery and Orthopedics share more common terminology due to their overlapping areas of treatment procedures.Whereas, Radiology, with its focus on imaging technology, likely uses a different set of specialized terms.
...
Since common lemmas are shared across various surgical procedures, their frequency is higher and hence they do not stand out. The “tf-idf” value combines term frequency with rarity across documents, which is low for these common lemmas.

For Orthopedic specialty, tf-idf values are relatively similar and low across all lemmas indicating that most of the lemmas may be common. This is why none of the lemmas in Orthopedic specialty stands out. Radiology, on the other hand, have varying tf-idf values and relatively higher values compared to Orthopedic or Surgery, which means it has more unique lemmas specific to Radiology. In Radiology, the lemma "left ventricular" stands out with high frequency by low tf-idf value among the other Radiology lemmas. Similarly, Surgery has very high frequency for its lemmas and some of its lemmas have high tf-idf values. Here, "general endotracheal" and "fascia was" stands out for similar reasons.
