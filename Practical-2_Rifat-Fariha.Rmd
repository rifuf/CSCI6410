...
tokenized.data_filtered <- tokenized.data.bigram %>%
  filter(!(str_detect(bigram, paste(stop_words, collapse = "|")) | str_detect(bigram, "[0-9]")))

# Count the unique bigrams for each medical specialty
unique_bigrams <- tokenized.data_filtered %>%
  group_by(medical_specialty) %>%
